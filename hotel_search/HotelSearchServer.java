 ...
 * 支持阿里云部署，可打包成JAR文件
 */
public class HotelSearchServer {
    ...
    // 支持多种数据源路径
    private static final String[] EXCEL_FILE_PATHS = {
        "日本东京酒店v2.xlsx",
        "../日本东京酒店v2.xlsx",
        "./data/日本东京酒店v2.xlsx",
        "/opt/hotel-search/data/日本东京酒店v2.xlsx"
    };
    ...
    private static void loadHotelsFromExcel() {
        ...
        // 尝试多个可能的文件路径
        for (String path : EXCEL_FILE_PATHS) {
        ...
        if (excelFilePath == null) {
            System.out.println("未找到Excel文件，使用示例数据...");
            loadSampleData();
            return;
        }
        
        try {
            System.out.println("正在从Excel文件加载酒店数据: " + excelFilePath);
    ...
    public static void main(String[] args) throws IOException {
        ...
        System.out.println("=== 启动酒店搜索服务器（可部署版本） ===");
        System.out.println("服务器地址: " + HOST + ":" + serverPort);
        ...
        HttpServer server = HttpServer.create(new InetSocketAddress(HOST, serverPort), 0);
        ...
        System.out.println("📝 测试命令: curl \"http://localhost:" + serverPort + "/api/v1/hotel/suggest?q=东京\"");
        System.out.println("🏥 健康检查: curl \"http://localhost:" + serverPort + "/health\"");
    ...
    static class StatsHandler implements HttpHandler {
        ...
        public void handle(HttpExchange exchange) throws IOException {
            ...
            try {
                ...
                response.put("deployment", "aliyun");
    ...
    static class HealthHandler implements HttpHandler {
        ...
        public void handle(HttpExchange exchange) throws IOException {
            ...
            try {
                ...
                response.put("service", "hotel-search");
                response.put("version", "1.0.0");
                
                sendJsonResponse(exchange, 200, response.toString());
                
            } catch (Exception e) {
                System.err.println("健康检查错误: " + e.getMessage());
                sendResponse(exchange, 500, "Internal Server Error");
            }
        }
    }
